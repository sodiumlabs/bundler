...
func PrivateMode() {
	...
	if chain.Cmp(config.ArbitrumOneChainID) == 0 || chain.Cmp(config.LumiterraChainID) == 0 || chain.Cmp(config.ArbitrumGoerliChainID) == 0 {
	...
	c.SetGetGasEstimateFunc(client.GetGasEstimateNoTraceWithEthClient(
		eoa,
		rpc,
		ov,
		chain,
